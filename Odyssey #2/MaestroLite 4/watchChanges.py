...
def takeScreenshot(imgPath):
    timestamp = (datetime.datetime.now()).strftime('%Y-%b-%d %H-%M-%S')
    imgFile = str('Screenshot_{0}.png'.format(timestamp))
    pic = pyautogui.screenshot()
    # Save the image
    pic.save(str(imgPath+imgFile))
    print('image  saved in {0}{1}'.format(imgPath, imgFile))
...
def datFileToDict(path,readFile):  #function outputs an associative array for the Odyssey UserVars.dat file
    ...
    keys = []
    for j in range(7, 36,4): #range(start,stop,step)
        key = lineArray[j].split("=")
        key.pop(0)
        keys.extend(key)
    values = []
    for k in range(86,94):
        ...
        if different == True:
            ...
            if cmp(currentRow,lastRow)!=0:
                ...
                try:
                    ...
                    print("Unable to copy file. %s \n Make sure .csv file is closed." % e)
                    exit(1)


if __name__ == "__main__":
    with open("micross.txt", "r") as file:
        print(file.read())
